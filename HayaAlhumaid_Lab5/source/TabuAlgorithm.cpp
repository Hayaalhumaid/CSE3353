...
std::pair<float, std::deque<int>> TabuAlgorithm::tabu_search(Graph * g, int src) {
	...
	nbd_size = g->map.size();
    current.first = std::numeric_limits<float>::max();
    current.second.clear();
    global.first = std::numeric_limits<float>::max();
    global.second.clear();
    ...
    for(size_t i=0; i<temporary_vector.size();i++){
...
void TabuAlgorithm::gen_nbd() {
    ...
    if(nbd_criteria == 0){
        ...
        while (nbd_ind < nbd_size){
            ...
            for (size_t i=0; i<use_random.size(); ++i) {
