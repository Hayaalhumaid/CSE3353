...
void SearchStrategy::Load(std::string filetype)
{
    graph_list.clear();
    graph_matrix.clear();
    weights.clear();
    ...
    std::ifstream weights_file("weights"+filetype);
    ...
    std::ifstream positions_file("positions"+filetype);
...
void SearchStrategy::Execute(int source, int destination)
{
    ...
    for (size_t i = 0; i < random_source_destination.size()+1; ++i)
    {
        {
            high_resolution_clock::time_point t1 = high_resolution_clock::now(); // remember starting time
            statistic_data_list_sd[i].explored = implementation->SearchList(graph_list, weights, positions, source, destination);
            ...
            statistic_data_list_sd[i].time = elapsed.count() + 1; // convert to integer

            std::vector<int> parents_list = implementation->GetParents();
            _CalcPathData(statistic_data_list_sd[i], parents_list, source, destination);

            t1 = high_resolution_clock::now();
            statistic_data_matrix_sd[i].explored = implementation->SearchMatrix(graph_matrix, weights, positions, source, destination);
            t2 = high_resolution_clock::now();
            elapsed = duration_cast<microseconds>(t2-t1);
            statistic_data_matrix_sd[i].time = elapsed.count() + 1;

            std::vector<int> parents_matrix = implementation->GetParents();
            _CalcPathData(statistic_data_matrix_sd[i], parents_matrix, source, destination);
        }
        {
            high_resolution_clock::time_point t1 = high_resolution_clock::now(); // remember starting time
            statistic_data_list_ds[i].explored = implementation->SearchList(graph_list, weights, positions, source, destination);
            high_resolution_clock::time_point t2 = high_resolution_clock::now(); // remember finishing time
            auto elapsed = duration_cast<microseconds>(t2-t1); // calculate elapsed time
            statistic_data_list_ds[i].time = elapsed.count() + 1; // convert to integer

            std::vector<int> parents_list = implementation->GetParents();
            _CalcPathData(statistic_data_list_ds[i], parents_list, source, destination);

            t1 = high_resolution_clock::now();
            statistic_data_matrix_ds[i].explored = implementation->SearchMatrix(graph_matrix, weights, positions, source, destination);
            t2 = high_resolution_clock::now();
            elapsed = duration_cast<microseconds>(t2-t1);
            statistic_data_matrix_ds[i].time = elapsed.count() + 1;

            std::vector<int> parents_matrix = implementation->GetParents();
            _CalcPathData(statistic_data_matrix_ds[i], parents_matrix, source, destination);
        }
...
{
    const ExecutionData& data = statistic_data_list_sd[0];
...
//        for (size_t i = 1; i < random_source_destination.size()+1; ++i)
//        {
...
//        }
        ...
        fout << std::min_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.path.size()<b.path.size();})->path.size() << "\t";
        fout << (double)std::accumulate(data.begin()+1, data.end(), 0, [](double a, const ExecutionData & b){return a + b.path.size();}) / (data.size()-1) << "\t";
        fout << std::max_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.path.size()<b.path.size();})->path.size() << "\n";

        fout << "Nodes explored (min/avr/max)\t";
        fout << std::min_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.explored<b.explored;})->explored << "\t";
        fout << (double)std::accumulate(data.begin()+1, data.end(), 0, [](double a, const ExecutionData & b){return a + b.explored;}) / (data.size()-1) << "\t";
        fout << std::max_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.explored<b.explored;})->explored << "\n";

        fout << "Execution time (min/avr/max)\t";
        fout << std::min_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.time<b.time;})->time << "\t";
        fout << (double)std::accumulate(data.begin()+1, data.end(), 0, [](double a, const ExecutionData & b){return a + b.time;}) / (data.size()-1) << "\t";
        fout << std::max_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.time<b.time;})->time << "\n";

        fout << "Path distance (min/avr/max)\t";
        fout << std::min_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.distance<b.distance;})->distance << "\t";
        fout << (double)std::accumulate(data.begin()+1, data.end(), 0, [](double a, const ExecutionData & b){return a + b.distance;}) / (data.size()-1) << "\t";
        fout << std::max_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.distance<b.distance;})->distance << "\n";

        fout << "Path cost (min/avr/max)\t";
        fout << std::min_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.cost<b.cost;})->cost << "\t";
        fout << (double)std::accumulate(data.begin()+1, data.end(), 0, [](double a, const ExecutionData & b){return a + b.cost;}) / (data.size()-1) << "\t";
        fout << std::max_element(data.begin()+1, data.end(), [](const ExecutionData &a, const ExecutionData &b){return a.cost<b.cost;})->cost << "\n";
